...
若用户要求保存文件，最终 Markdown 推荐写到当前项目或仓库下的 `reports/` 目录，例如 `reports/ai-daily-YYYY-MM-DD.md` 或 `reports/ai-weekly-YYYY-MM-DD.md`。
...
1. 收集候选：
```bash
python3 <SKILL_DIR>/scripts/report.py daily --output-json /tmp/ai-daily-candidates.json
```
或：
```bash
python3 <SKILL_DIR>/scripts/report.py weekly --output-json /tmp/ai-weekly-candidates.json
```
2. 脚本会根据模式选择来源：
- 日报和周报来源都从 `config/sources.json` 读取。
- 日报默认包含 `news.smol.ai`、`news.ycombinator.com`、`hn.algolia.com`、`app.daily.dev/agents`、`ai-bot.cn/daily-ai-news`、`news.aibase.com/zh/news`、`maomu.com/news`、`github.com/trending`、`collector-search-recall`。
...
指定日报日期：
```bash
python3 <SKILL_DIR>/scripts/report.py daily --date 2026-04-08 --output-json /tmp/ai-daily-candidates.json
```

最近 24 小时：
```bash
python3 <SKILL_DIR>/scripts/report.py daily --from 2026-04-08T12:00:00+08:00 --to 2026-04-09T12:00:00+08:00
```
